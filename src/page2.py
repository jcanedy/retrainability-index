...
df = pd.read_csv("data/processed/index_tier1.csv", low_memory=False)

with st.expander("Index Configurations"):

    weights = np.array([1, 1, 1, 1]) / 4

    weight1 = st.slider("bin_r_cog_y mean", 0.0, 1.0, value=weights[0])
    weight2 = st.slider("bin_r_man_y mean", 0.0, 1.0, value=weights[1])
    weight3 = st.slider("bin_offshor_y mean", 0.0, 1.0, value=weights[2])
    ...
    df["index_y"] = (
        normalized_weights[0] * df["bin_r_cog_y mean"] + 
        normalized_weights[1] * df["bin_r_man_y mean"] + 
        normalized_weights[2] * df["bin_offshor_y mean"] +
